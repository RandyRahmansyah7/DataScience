# Retail Profitability & Discount Strategy Analysis

Executive dashboard built with Streamlit + Plotly, Clean Corporate design.

## Run Locally

```bash
pip install -r requirements.txt
streamlit run app.py
```

## Deploy to Streamlit Cloud (Free)

1. Push this folder to a GitHub repo (include `app.py`, `requirements.txt`, and `SampleSuperstore.csv`)
2. Go to https://share.streamlit.io
3. Connect your GitHub repo, select `app.py` as the main file
4. Deploy — you'll get a public URL to share

## Files
- `app.py` — main dashboard application
- `Sample_-_Superstore.csv` — dataset (must stay in the same folder as app.py)
- `requirements.txt` — Python dependencies
